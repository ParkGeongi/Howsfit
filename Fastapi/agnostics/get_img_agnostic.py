...
def getting_img_agnostic(model_fname):
    ...
    for im_name in tqdm(os.listdir(osp.join(data_path, 'image', model_fname.split('.')[0]))):
        print(f"##### get img agnostic : im_name is {im_name}")
        # load pose image
        ...
        # load parsing image
        im = Image.open(osp.join(data_path, 'image', model_fname.split('.')[0], im_name))
